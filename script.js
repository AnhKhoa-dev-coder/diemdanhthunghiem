...
// BẬT CAMERA – CHỈ SAU KHI BẤM NÚT
...
async function startAttendance() {
  ...
  // Giảm kích thước canvas để mobile không lag
  const w = 320;
  ...
  const imageData = canvas.toDataURL("image/jpeg", 0.7); // nén ảnh

  fetch("http://127.0.0.1:5000/attendance", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ code, image: imageData }),
  })
    .then((res) => res.json())
    .then((data) => {
      ctx.clearRect(0, 0, canvas.width, canvas.height);
...
function loadToday() {
  fetch("http://127.0.0.1:5000/today")
    ...
    .then((data) => {
      ...
      if (data.length === 0) {
        tableBody.innerHTML = "<tr><td colspan='5'>Chưa có dữ liệu</td></tr>";
        return;
      }
